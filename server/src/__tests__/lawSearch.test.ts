...
describe('searchLaws — edge cases', () => {
  ...
  it('повертає не більше 10 результатів за замовчуванням', () => {
    ...
    expect(результати.length).toBeLessThanOrEqual(15);
  });
});
...
describe('hybridSearchLaws — гібридний пошук', () => {
  ...
  it('комбінує keyword та vector результати з правильними вагами', async () => {
    ...
    expect(результати.length).toBeGreaterThan(0);
    // rerank викликається з кандидатами
...
describe('hybridSearchLaws — re-ranking інтеграція', () => {
  ...
  it('re-ranker змінює порядок результатів', async () => {
    ...
    // Re-ranker ставить test-st3-ch1 на перше місце
  ...
  it('rerank викликається з текстами документів', async () => {
    mockСтворитиЕмбеддинг.mockResolvedValue(new Array(384).fill(0.1));
    mockПошукПоВектору.mockResolvedValue([
      {
        id: 'test-st2-ch1',
    ...
    mockRerank.mockResolvedValue([{ id: 'test-st2-ch1', score: 0.8 }]);
  ...
  it('graceful fallback якщо rerank кидає помилку', async () => {
    mockСтворитиЕмбеддинг.mockResolvedValue(new Array(384).fill(0.1));
    mockПошукПоВектору.mockResolvedValue([
      {
        id: 'test-st2-ch1',
    ...
    mockRerank.mockRejectedValue(new Error('Re-ranker помилка'));
  ...
  it('не викликає rerank якщо немає кандидатів', async () => {
    mockСтворитиЕмбеддинг.mockResolvedValue(new Array(384).fill(0.1));
    mockПошукПоВектору.mockResolvedValue([
      {
        id: 'test-st4-ch1',
