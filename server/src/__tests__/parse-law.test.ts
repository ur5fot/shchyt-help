...
describe('splitLargeChunks', () => {
  it('розбиває великий чанк по підпунктах', () => {
    ...
    const longItems = Array.from({ length: 30 }, (_, i) =>
      `${i + 1}) Підпункт номер ${i + 1} з достатньо довгим текстом щоб набрати потрібну кількість символів для розбивки великого чанка при парсингу закону про військовий обов'язок і військову службу в Збройних Силах України відповідно до чинного законодавства.`
    ).join(' ');
    const html = `<html><body>
      <h1>Тест</h1>
