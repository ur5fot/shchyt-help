...
describe('POST /api/chat', () => {
  ...
  it('не додає дисклеймер коли він вимкнений (порожній рядок)', async () => {
    ...
    expect(відповідь.status).toBe(200);
    // ДИСКЛЕЙМЕР зараз вимкнений — відповідь залишається без змін
    expect(відповідь.body.answer).toBe('Відповідь без попередження');
  });

  it.skipIf(!ДИСКЛЕЙМЕР)('додає дисклеймер якщо AI його пропустив', async () => {
    mockAskClaude.mockResolvedValue('Відповідь без попередження');
  ...
  it.skipIf(!ДИСКЛЕЙМЕР)('не дублює дисклеймер якщо AI його вже додав', async () => {
