  ...
  try {
    ...
    if (sanitizedHistory && sanitizedHistory.length > 0) {
      ...
      if (останнєПитання) {
        ...
        if (останняВідповідь) {
          const знайдені = останняВідповідь.content.match(/статт[а-яіїєґ']*\s+\d+(?:-\d+)?|частин[а-яіїєґ']*\s+\d+(?:-\d+)?|пункт[а-яіїєґ']*\s+\d+(?:-\d+)?|ст\.\s*\d+(?:-\d+)?|п\.\s*\d+(?:-\d+)?|ч\.\s*\d+(?:-\d+)?/gi);
          if (знайдені) {
        ...
        const повнийЗапит = `${trimmed}${статтіЗВідповіді} ${останнєПитання.content}`;
        const позиціяПробілу = повнийЗапит.lastIndexOf(' ', 400);
        const контекстнийЗапит = повнийЗапит.length <= 400
          ? повнийЗапит
          : повнийЗапит.slice(0, позиціяПробілу > 0 ? позиціяПробілу : 400);
    ...
    // Фільтруємо до підтверджених джерел тільки якщо є хоча б одна верифікована цитата.
    // Якщо блоку ЦИТАТИ не було, або жодна цитата не верифікована — повертаємо всі джерела (graceful degradation).
    // Це захищає від випадку коли hasCitationBlock і extractCitations мають різну строгість парсингу.
