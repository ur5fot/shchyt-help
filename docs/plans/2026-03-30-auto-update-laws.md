...
- [ ] Для кожного JSON файлу в `laws/`:
  - Завантажити HTML з `source_url`
  - Обчислити sha256
  - Порівняти з збереженим хешем
  - Якщо змінився — вивести повідомлення і запропонувати оновити
- [ ] Режим `--auto` — автоматично перепарсити змінені закони (без підтвердження):
  - Перепарсити HTML через `parseLaw()`
  - Згенерувати ембеддинги
  - Upsert в LanceDB
  - Оновити JSON файл
  - Оновити хеш
- [ ] Режим без `--auto` — тільки показати які закони змінились
- [ ] Логування: "Перевірено N законів, змінено M, оновлено K"
- [ ] Graceful: якщо rada.gov.ua недоступна — пропустити з warning
- [ ] Додати npm script: `"check-updates": "tsx scripts/check-updates.ts"` в root package.json
...
- [ ] Оновити `CLAUDE.md` — додати опис `npm run check-updates`
- [ ] Оновити `README.md` — додати команду
- [ ] `npm test` — всі тести проходять
