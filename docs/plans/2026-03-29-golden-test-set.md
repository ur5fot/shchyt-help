...
- [ ] Створити директорію `eval/`
- [ ] Створити `eval/golden-set.json` з 50+ питаннями у форматі:
```json
[
  {
    "id": "vacation-basic",
...
```
- [ ] Покрити всі основні категорії:
  - Відпустки (основна, додаткова, лікування, Чорнобиль) — 8 питань
  - Грошове забезпечення (оклад, бойові, затримки) — 8 питань
  - Звільнення/демобілізація — 6 питань
  - Мобілізація (відстрочки, бронювання) — 6 питань
...
- [ ] Кожне питання має мати мінімум 1 expectedChunk (ID чанка з laws/*.json)
- [ ] Перевірити що всі expectedChunk ID реально існують у базі
