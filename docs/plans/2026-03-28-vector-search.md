...
- [x] імпортувати `створитиЕмбеддинг` з `embeddings.ts` та `пошукПоВектору` з `vectorStore.ts`
- [x] нова async функція `hybridSearchLaws(запит, чанки): Promise<SearchResult[]>`
...
- [x] додати константи в `constants.ts`: `ВАГА_КЛЮЧОВИХ_СЛІВ = 0.4`, `ВАГА_ВЕКТОРА = 0.6`, `МІНІМАЛЬНА_ГІБРИДНА_ОЦІНКА = 0.15`
- [x] написати тести для гібридного пошуку
- [x] `npm test` — всі тести проходять
