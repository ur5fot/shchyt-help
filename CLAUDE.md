...
eval/        → Golden test set (66 питань з очікуваними чанками та статтями)
```
...
- **scripts/eval.ts** — скрипт оцінки якості: `npm run eval` (retrieval recall по golden set, 66 питань), `npm run eval -- --full` (повний eval з Claude API: citation accuracy, fact recall)
...
13 документів, ~2517 чанків — покривають ~95% типових питань:
...
66 питань у `eval/golden-set.json` з очікуваними чанками та статтями по 11 категоріях. `npm run eval` вимірює retrieval recall (keyword), `npm run eval -- --full` додає citation accuracy, hallucination rate та fact recall через Claude API. Метрики реалізовані в `evalMetrics.ts`.
