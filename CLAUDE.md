...
eval/        → Golden test set (63 питання з очікуваними чанками та статтями)
```
...
- **scripts/eval.ts** — скрипт оцінки якості: `npm run eval` (retrieval recall по golden set, 63 питання), `npm run eval -- --full` (повний eval з Claude API: citation accuracy, fact recall)
...
- **scripts/parse-nakaz40.ts** — парсер Наказу МОУ №40 (Інструкція з діловодства ЗСУ) з .docx або plain text: пункти, підпункти, розділи → JSON чанки (631 чанків). Підтримує 4-рівневу нумерацію (N.M.K.L), розбиває inline і standalone пункти, обробляє "плоскі" розділи без підпунктів. Запуск: `npx tsx scripts/parse-nakaz40.ts [шлях-до-docx-або-txt]`

## База законів

12 документів, ~2508 чанків — покривають ~95% типових питань:
...
12. Наказ МОУ №40 «Інструкція з діловодства у ЗСУ» (зі змінами від 27.02.2026) — правила оформлення рапортів, скарг, реквізити документів, підписи, бланки, реєстрація (631 чанків)
