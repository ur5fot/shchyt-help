...
- **База законів:** JSON-файли (без БД)
...
scripts/   → Парсер законів (HTML з zakon.rada.gov.ua → JSON-чанки)
```

**Потік запиту:** Питання користувача → `lawSearch.ts` знаходить релевантні чанки законів через пошук по ключових словах → `promptBuilder.ts` складає промпт з контекстом законів → `claude.ts` відправляє в Claude API → відповідь з цитатами та джерелами повертається клієнту.
...
- **lawSearch.ts** — пошук по законах з нормалізацією (стемінг українських слів) та картою синонімів. Мінімальний поріг релевантності = 3 бали
