...
- **scripts/parse-nakaz40.ts** — парсер Наказу МОУ №40 (Інструкція з діловодства ЗСУ) з PDF-тексту: пункти, підпункти, розділи → JSON чанки (541 чанк). Запуск: `npx tsx scripts/parse-nakaz40.ts [шлях-до-txt]`

## База законів

13 документів, ~2418 чанків — покривають ~95% типових питань:
...
12. Наказ МОУ №40 «Інструкція з діловодства у ЗСУ» — правила оформлення рапортів, скарг, реквізити документів, підписи, бланки, реєстрація (541 чанк)
