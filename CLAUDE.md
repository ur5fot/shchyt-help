...
LLM-згенероване резюме (1-2 речення) додано до всіх ~3100 чанків через `scripts/generate-summaries.ts` (Claude Sonnet, ~$0.50 за всю базу). Summary використовується в keyword search (вага 2), vector embeddings та Claude re-ranking. Keyword recall покращився з 44.1% до 52.9%.
