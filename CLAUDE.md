...
- **scripts/parse-nakaz40.ts** — парсер Наказу МОУ №40 (Інструкція з діловодства ЗСУ) з PDF-тексту: пункти, підпункти, розділи → JSON чанки (630 чанків). Підтримує 4-рівневу нумерацію (N.M.K.L) та розбиває inline і standalone пункти. Запуск: `npx tsx scripts/parse-nakaz40.ts [шлях-до-txt]`

## База законів

12 документів, ~2507 чанків — покривають ~95% типових питань:
...
12. Наказ МОУ №40 «Інструкція з діловодства у ЗСУ» — правила оформлення рапортів, скарг, реквізити документів, підписи, бланки, реєстрація (630 чанків)
