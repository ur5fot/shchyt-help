...
**Потік запиту:** Питання користувача → `lawSearch.ts` знаходить релевантні чанки через гібридний пошук (keyword + vector similarity через LanceDB + HyDE hypothesis vector search) + Claude Sonnet re-ranking (top-50 → top-25, fallback на bge cross-encoder) → `promptBuilder.ts` складає промпт з контекстом законів → `claude.ts` відправляє в Claude API → `citationVerifier.ts` перевіряє цитати AI проти наданих чанків → відповідь з верифікованими джерелами повертається клієнту. Якщо LanceDB не ініціалізована — автоматичний fallback на keyword пошук.
...
### ~~4. Chunk enrichment (збагачення чанків)~~ — ЧАСТКОВО РЕАЛІЗОВАНО
LLM-згенероване резюме (1-2 речення) додано до всіх ~3100 чанків через `scripts/generate-summaries.ts`. Summary використовується в keyword search, vector embeddings та Claude re-ranking. Залишається: повний ієрархічний шлях (закон → розділ → стаття → частина).
