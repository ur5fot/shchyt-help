...
59 питань у `eval/golden-set.json` з очікуваними чанками та статтями по 10 категоріях. `npm run eval` вимірює retrieval recall (keyword), `npm run eval -- --full` додає citation accuracy, hallucination rate та fact recall через Claude API. Метрики реалізовані в `evalMetrics.ts`.
